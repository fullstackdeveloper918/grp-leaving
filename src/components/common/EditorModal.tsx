...
const EditorModal: React.FC = () => {
    ...
    useEffect(() => {
      if (searchTerm) {
        fetch(`https://api.tenor.com/v1/search?q=${searchTerm}&key=${TENOR_API_KEY}&limit=5`)
          .then((response) => response.json())
          .then((data) => {
            const gifUrls = data.results.map((result: any) => result.media[0].gif.url);
            setGifs(gifUrls);
          })
    ...
    const handleMediaUpload = (files: FileList, type: "image" | "gif" | "sticker") => {
      if (files) {
      ...
      <div className="editor_app">
        ...
        <div className="editor_option">
          ...
          <div className="search_input">
            ...
            <svg className="MuiSvgIcon-root MuiSvgIcon-fontSizeMedium mus-vubbuv" focusable="false" aria-hidden="true" viewBox="0 0 24 24" data-testid="AddPhotoAlternateIcon"><path d="M19 7v2.99s-1.99.01-2 0V7h-3s.01-1.99 0-2h3V2h2v3h3v2zm-3 4V8h-3V5H5c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2v-8zM5 19l3-4 2 3 3-4 4 5z"></path></svg>
            </div>
          </div>
